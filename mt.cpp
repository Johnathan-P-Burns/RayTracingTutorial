...
{
	std::size_t x = index % dimX;
	std::size_t y = index / dimX;

	vec3 col(0,0,0);

	for(std::size_t sample = 0; sample < samples; sample++)
	{
		float u = float(x + drand48()) / float(dimX);
		float v = float(y + drand48()) / float(dimY);
	...
	return new vec3(sqrt(col.r()), sqrt(col.g()), sqrt(col.b()));
}
...
int main()
{
	...
	*/
	...
	for(int j = dimY - 1; j >= 0; j--)
	{
		...
		{
		int r = int(255.99 * image[i + dimX * j]->r());
		int g = int(255.99 * image[i + dimX * j]->g());
		int b = int(255.99 * image[i + dimX * j]->b());
